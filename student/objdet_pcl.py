...
def show_pcl(pcl):
    ...
    ## Making use of content mostly from lesson 2.18 and from https://knowledge.udacity.com/questions/725173
...
def show_range_image(frame, lidar_name):
    ...
    ## Making use of content mostly from lesson 2.17.
    ...
    # step 7 : align image center with forward driving direction
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    ## Making use of content mostly from lesson 3.6.
    ...
    _, idx_intensity_unique = np.unique(lidar_pcl_cpy[:, 0:2], axis=0, return_index=True)
    lidar_pcl_int = lidar_pcl_cpy[idx_intensity_unique]

    ## step 4 : assign the intensity value of each unique entry in lidar_top_pcl to the intensity map 
    ##          make sure that the intensity is scaled in such a way that objects of interest (e.g. vehicles) are clearly visible    
    ##          also, make sure that the influence of outliers is mitigated by normalizing intensity on the difference between the max. and min. value within the point cloud
    idx_limit = lidar_pcl_int[:,3] > 1.0
    lidar_pcl_int[idx_limit, 3] = 1.0
    ...
    show_intensty_map = False
    if show_intensty_map:
